...
PIDSTAT_INTERVAL=0.1   # not used in robust approach but kept for compatibility notes
...
progress "Measurements starting. Results -> ${PERF_DIR}/"
...
for size in "${SIZES[@]}"; do
  ...
  for ((c=1; c<=CALLGRIND_RUNS; c++)); do
    ...
    # run callgrind quietly: valgrind tool writes to stderr; capture that to a log file
  ...
  if command -v callgrind_annotate >/dev/null 2>&1 && [ -f "${callgrind_out}" ]; then
    ...
    # heuristic snippet of top candidate functions into summary
  ...
  # -------------------------
  # 3) Robust native runs (sampling via ps + /proc/<pid>/stat)
  # -------------------------
